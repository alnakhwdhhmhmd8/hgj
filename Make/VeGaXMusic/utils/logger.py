...
async def play_logs(message, streamtype):
    if await is_on_off(2):
        ...
        if message.chat.type == ChatType.CHANNEL:
            logger_text = f"""━━━━ılıı◁ ❚ ᴄʜᴧɴɴᴇʟ ❚  ▷ıılı━━━━
<blockquote><b>╭⚉<b>{app.mention}
<b>╰⚆ᴘʟᴧʏ ⸢ᴄʜᴧɴɴᴇʟ⸥ ꜱʜᴇɪᴋʜ♪</b>
...
<b>╰❖ ᴜsᴇꝛɴᴧᴍᴇ :</b> @{message.from_user.username if message.from_user and message.from_user.username else "Unknown User"}

<b>╭❖ ǫᴜᴇꝛʏ :</b> {message.text.split(None, 1)[1]}
<b>╰❖ sᴛꝛᴇᴧᴍᴛʏᴘᴇ :</b> {streamtype}
</blockquote>━ılıılıılıılıılıılıılıılıılıılıılıılılıılıılıılıılıılıılıılı━"""
            reply_markup = InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
