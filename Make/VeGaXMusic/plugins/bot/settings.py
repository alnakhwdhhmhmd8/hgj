...
from VeGaXMusic.utils.decorators.admins import ActualAdminCB
from VeGaXMusic.utils.decorators.language import language, languageCB
from VeGaXMusic.utils.inline.settings import (
...
from VeGaXMusic.utils.inline.start import private_panel
from VeGaXMusic.misc import SUDOERS
### Command
SETTINGS_COMMAND = get_command("SETTINGS_COMMAND")

@app.on_message( filters.command(["الاعدادات","اعدادات"], "") & filters.group & ~BANNED_USERS, group=136676) 

@app.on_message(filters.command(SETTINGS_COMMAND) & filters.group & ~BANNED_USERS, group=348970)
@language
...
@app.on_callback_query(filters.regex("settings_helper") & ~BANNED_USERS, group=378004)
@languageCB
...
@app.on_callback_query(filters.regex("settingsback_helper") & ~BANNED_USERS, group=3199084)
@languageCB
...
@app.on_callback_query(
    filters.regex(pattern=r"^(CLEANMODE|COMMANDELMODE)$") & ~BANNED_USERS, group=39994
)
@ActualAdminCB
...
# Auth Users Settings
@app.on_callback_query(filters.regex(pattern=r"^(AUTH|AUTHLIST)$") & ~BANNED_USERS, group=9804)
@ActualAdminCB
