...
from VeGaXMusic.misc import HAPP, SUDOERS, XCB
...
from VeGaXMusic.utils.pastebin import KIMbin
...
@app.on_message(filters.command(GETVAR_COMMAND) & SUDOERS, group=34098)
@language
...
@app.on_message(filters.command(DELVAR_COMMAND) & SUDOERS, group=34765)
@language
async def vardel_(client, message, _):
    ...
    if await is_heroku():
        ...
        if not output[0]:
            ...
            await message.reply_text(_["heroku_7"].format(check_var))
            os.system(f"kill -9 {os.getpid()} && python3 -m VeGaXMusic")


@app.on_message(filters.command(SETVAR_COMMAND) & SUDOERS, group=354454)
@language
...
@app.on_message(filters.command(USAGE_COMMAND) & SUDOERS, group=3476600)
@language
...
@app.on_message(filters.command(["update", "gitpull", "up"]) & SUDOERS, group=376664)
@language
...
@app.on_message(filters.command(["restart"]) & SUDOERS, group=347760)
