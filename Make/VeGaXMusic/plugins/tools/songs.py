...
from VeGaXMusic.utils.formatters import convert_bytes
from VeGaXMusic.utils.inline.song import song_markup

# Command

SONG_COMMAND = get_command("SONG_COMMAND")
@app.on_message(filters.command(SONG_COMMAND) & filters.private & ~BANNED_USERS, group=7634)
@language
    ...
    if url:
        ...
        if int(duration_sec) > SONG_DOWNLOAD_DURATION_LIMIT:
            return await mystic.edit_text(
                _["play_4"].format(SONG_DOWNLOAD_DURATION_LIMIT, duration_min)  # Fixed variable name
            )
        ...
        if int(duration_sec) > SONG_DOWNLOAD_DURATION_LIMIT:
            return await mystic.edit_text(
                _["play_6"].format(SONG_DOWNLOAD_DURATION_LIMIT, duration_min)  # Fixed variable name
            )
...
@app.on_callback_query(filters.regex(pattern=r"song_back") & ~BANNED_USERS, group=35604)
@languageCB
...
@app.on_callback_query(filters.regex(pattern=r"song_helper") & ~BANNED_USERS, group=309874)
@languageCB
...
@app.on_callback_query(filters.regex(pattern=r"song_download") & ~BANNED_USERS, group=3094)
@languageCB
async def song_download_cb(client, CallbackQuery, _):
    ...
    try:
        ...
        await mystic.edit_text(_["song_10"])
        print(f"Error in song_download_cb: {e}")