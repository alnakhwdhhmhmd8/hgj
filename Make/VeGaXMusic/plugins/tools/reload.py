...
from VeGaXMusic.utils.database import get_authuser_names, get_cmode
from VeGaXMusic.utils.decorators import ActualAdminCB, AdminActual, language
from VeGaXMusic.utils.formatters import alpha_to_int
...
@app.on_message(filters.command(RELOAD_COMMAND) & filters.group & ~BANNED_USERS, group=343422)
@language
...
@app.on_message(filters.command(REBOOT_COMMAND) & filters.group & ~BANNED_USERS, group=31114)
@AdminActual
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await KIM.stop_stream(message.chat.id)
    except:
        pass
...
@app.on_callback_query(filters.regex("close") & ~BANNED_USERS, group=7777777665566)
async def close_menu(_, CallbackQuery):
    try:
        await CallbackQuery.answer()
        await CallbackQuery.message.delete()
        await CallbackQuery.message.reply_text(
            f"<b>⦿ تـم حذف رسالـه مـن:\n│ \n╰⦿: {CallbackQuery.from_user.mention}</b>"
        )
    except:
        pass


@app.on_callback_query(filters.regex("stop_downloading") & ~BANNED_USERS, group=34334)
@ActualAdminCB
